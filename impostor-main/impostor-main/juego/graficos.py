...
class VentanaJuego:
    ...
    def pedir_palabra(self):
        ...
        if self.jugador_actual == self.partida.impostores_index[0]:
            tk.Label(
                self.frame,
    ...
    def votar_expulsar(self, indice_expulsado):
        ...
        if self.jugador_actual < self.partida.num_jugadores:
            ...
            civiles_ganan = self.partida.resultado_votacion()[1]
            
            self.mostrar_resultado(expulsado, civiles_ganan)
    
    def mostrar_resultado(self, expulsado_idx, civiles_ganan):
